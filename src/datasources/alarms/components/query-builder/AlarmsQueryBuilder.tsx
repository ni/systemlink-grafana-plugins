...
type AlarmsQueryBuilderProps = QueryBuilderProps & React.HTMLAttributes<Element> & {
  filter?: string;
...
export const AlarmsQueryBuilder: React.FC<AlarmsQueryBuilderProps> = ({
  filter,
  onChange,
  workspaces,
  partNumbers,
  ...
  const workspaceField = useMemo(() => {
    const workspaceField = AlarmsQueryBuilderFields.WORKSPACE;
  ...
  const updatedAtField = useMemo(() => {
    const updatedField = AlarmsQueryBuilderFields.UPDATEDAT;
    return {
      ...updatedField,
      lookup: {
        ...updatedField.lookup,
  ...
  const partNumberField = useMemo(() => {
    const partNumberField = AlarmsQueryBuilderFields.PARTNUMBER;
  ...
  const familyField = useMemo(() => {
          const familyField = AlarmsQueryBuilderFields.FAMILY;
          return {
              ...familyField,
              lookup: {
                  ...familyField.lookup,
  ...
  useEffect(() => {
    const updatedFields = [partNumberField, familyField, ...AlarmsQueryBuilderStaticFields!, updatedAtField, workspaceField]
      .map((field) => {
