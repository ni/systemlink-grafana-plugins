...
describe('ResultsQueryEditor', () => {
  beforeEach(async() => {
    [onChange] = render({ refId: '', outputType: 'Data', properties: [], orderBy: undefined, descending: false, recordCount:1000, useTimeRange: true, useTimeRangeFor: undefined} as ResultsQuery);
  ...
  test('updates when user makes changes', async () => {
    ...
    await waitFor(() => {
      expect(onChange).toHaveBeenCalledWith(
        expect.objectContaining({ properties: ["properties"] })
      )
    });
    ...
    await waitFor(() => {
      expect(onChange).toHaveBeenCalledWith(
        expect.objectContaining({ orderBy: "STARTED_AT" })
      )
    });
    ...
    await waitFor(() => {
      expect(onChange).toHaveBeenCalledWith(
        expect.objectContaining({ useTimeRange: false })
      )
    });
    ...
    await waitFor(() => {
      expect(onChange).toHaveBeenCalledWith(
        expect.objectContaining({ useTimeRangeFor: "Updated" })
      )
    });
