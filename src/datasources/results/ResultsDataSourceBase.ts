...
export abstract class ResultsDataSourceBase extends DataSourceBase<ResultsQuery> {
  ...
  async loadWorkspaces(): Promise<Map<string, Workspace>> {
    ...
    ResultsDataSourceBase._workspacesCache = this.getWorkspaces()
      ...
      .catch(error => {
        if (!this.error) {
          this.handleQueryResultValuesError(error, 'result');
        }
  ...
  handleQueryResultValuesError(error: unknown, errorContext: string): void {
    ...
    this.error = `Warning during ${errorContext} value query`;
